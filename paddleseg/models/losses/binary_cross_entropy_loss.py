...
class SoftBCELoss(nn.Layer):
    ...
    def forward(self, logit, label, hard_label):
        """
        Forward computation.
        ...
        if len(label.shape) != len(logit.shape):
            label = paddle.unsqueeze(label, 1)
        label = paddle.nn.functional.sigmoid(label)
        mask = (hard_label != self.ignore_index)
        mask = paddle.cast(mask, 'float32')
        # label.shape should equal to the logit.shape
        if label.shape[1] != logit.shape[1]:
            label = label.squeeze(1)
        ...
        loss = loss * mask
        loss = paddle.mean(loss) / (paddle.mean(mask) + self.EPS)
        # label.stop_gradient = True # soft label should transmit gradient
        mask.stop_gradient = True
